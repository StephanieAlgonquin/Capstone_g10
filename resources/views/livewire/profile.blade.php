...
<div class="min-h-screen bg-[#1A1B23] text-white flex">
    ...
    <aside class="w-20 bg-[#252732] flex flex-col items-center py-6 border-r border-[#3A3B47]">
        ...
        <a href="{{ route('profile') }}" class="mb-4 p-3 text-white bg-[#353642] rounded-lg" title="Settings">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <header class="bg-[#252732] border-b border-[#3A3B47] px-6 py-4">
            <div class="flex items-center justify-between">
                <div class="flex flex-row gap-1">
                    <img src="{{ asset('taskmaker-logo.svg') }}" alt="TaskMaker Logo" class="w-10 h-10 sm:h-32 sm:w-32">
                ...
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold overflow-hidden {{ Auth::user()->avatar ? '' : 'bg-blue-600' }}">
                    ...
                    <a href="{{ route('dashboard') }}" class="px-4 py-2 bg-[#353642] text-white rounded-lg hover:bg-[#3A3B47] transition-colors text-sm">
                        Back to Dashboard
                    </a>
                </div>
            </div>
...
{{--                            <div class="pt-4 border-t border-[#3A3B47]">--}}
{{--                                <label class="block text-sm font-medium text-gray-300 mb-3">Profile Photo</label>--}}
{{--                                <div class="flex items-center gap-6">--}}
{{--                                    <!-- Profile Photo Display -->--}}
{{--                                    <div class="relative">--}}
{{--                                        @if($avatarPreview)--}}
{{--                                            <img src="{{ $avatarPreview }}" alt="Profile Photo" class="w-20 h-20 rounded-full object-cover border-4 border-[#353642]">--}}
{{--                                        @else--}}
{{--                                            <div class="w-20 h-20 rounded-full bg-purple-600 flex items-center justify-center text-white text-2xl font-bold border-4 border-[#353642]">--}}
{{--                                                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}--}}
{{--                                            </div>--}}
{{--                                        @endif--}}
...
{{--                                                class="absolute -top-1 -right-1 w-6 h-6 bg-red-600 rounded-full flex items-center justify-center text-white hover:bg-red-700 transition-colors"--}}
...
{{--                                                       class="inline-flex items-center px-4 py-2 bg-[#353642] border border-[#3A3B47] rounded-lg text-white hover:bg-[#3A3B47] transition-colors cursor-pointer">--}}
...
{{--                                                    <span wire:loading wire:target="uploadAvatar" class="flex items-center gap-2">--}}
...
{{--                                                        wire:loading.attr="disabled"--}}
{{--                                                        class="px-4 py-2 bg-[#353642] border border-[#3A3B47] text-white rounded-lg hover:bg-[#3A3B47] transition-colors text-sm font-medium">--}}
{{--                                                    Cancel--}}
{{--                                                </button>--}}
{{--                                            </div>--}}
{{--                                            @endif--}}
{{--                                            @error('avatar') --}}
{{--                                                <span class="text-red-400 text-xs block mt-1 font-medium">{{ $message }}</span> --}}
{{--                                            @enderror--}}
{{--                                            @if(session('error'))--}}
{{--                                                <span class="text-red-400 text-xs block mt-1 font-medium">{{ session('error') }}</span>--}}
{{--                                            @endif--}}
{{--                                            <p class="text-xs text-gray-500">JPG, PNG or GIF. Max size 5MB. Photo will be cropped to fit round icon.</p>--}}
{{--                                        </div>--}}
{{--                                    </div>--}}
{{--                                </div>--}}
{{--                            </div>--}}
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-[#2D2E3A] border border-[#3A3B47] rounded-lg p-6">
                        ...
                        <div class="space-y-6">
                            <div class="pb-4 border-b border-[#3A3B47]">
                                ...
                                <p class="text-3xl font-bold text-white">{{ Auth::user()->lists()->count() }}</p>
                            </div>
                            <div class="pb-4 border-b border-[#3A3B47]">
                                <p class="text-sm text-gray-400 mb-1">Total Tasks</p>
                                <p class="text-3xl font-bold text-white">
                                    {{ \App\Models\Task::whereHas('list', function($q) { $q->where('user_id', Auth::id()); })->count() }}
                                </p>
                            </div>
                            <div>
                    ...
                    <div class="bg-[#2D2E3A] border border-[#3A3B47] rounded-lg p-6">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('dashboard') }}" class="block w-full px-4 py-3 text-center bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
                                Go to Dashboard
                            </a>
