...
<div class="min-h-screen bg-[#1A1B23] text-white flex">
    ...
    <aside class="hidden md:flex w-20 bg-[#252732] flex-col items-center py-6 border-r border-[#3A3B47]">
        <a href="{{ route('tasks') }}" class="mb-8 p-3 bg-blue-600 rounded-lg" title="Tasks">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <main class="flex-1 overflow-x-auto p-6 bg-[#1A1B23]">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 h-full min-h-[600px]">
                ...
                @foreach($userLists as $tasklist)
                
                <div class="bg-[#2D2E3A] border rounded-lg p-4 flex flex-col border-1" style="border-color: {{ $tasklist->color }};">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-lg font-semibold text-white">{{ $tasklist->name }}</h2>
                        <div>
                        <button wire:click="openAddForm({{$tasklist->id}})" 
                                class="p-1 text-gray-400 hover:text-white hover:bg-[#353642] rounded transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                        </button>
                        <button wire:click="openEditForm({{$tasklist->id}})" 
                                class="p-1 text-gray-400 hover:text-white hover:bg-[#353642] rounded transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if(isset($showEditForm[$tasklist->id]) && $showEditForm[$tasklist->id])
                    <div class="mb-4 p-4 bg-[#353642] border border-purple-500/30 rounded-lg shadow-lg">
                        ...
                        <form wire:submit.prevent="updateList({{$tasklist->id}})" class="space-y-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-300 mb-2">List Name</label>
                                <input type="text" 
                                    wire:model="editingName.{{$tasklist->id}}" 
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-3">
                                    <input type="color" 
                                        wire:model="editingColor.{{$tasklist->id}}"
                                        class="w-14 h-10 bg-[#2D2E3A] border border-[#3A3B47] rounded-lg cursor-pointer">
                                    <span class="text-xs text-gray-400">
                                        {{ $editingColor[$tasklist->id] ?? $tasklist->color ?? '#6366F1' }}
                                    </span>
                                </div>
                            </div>
                    ...
                    <!-- Add Task Form -->
                    @elseif(isset($showAddForm[$tasklist->id]) && $showAddForm[$tasklist->id])
                    <div class="mb-4 p-3 bg-[#353642] border border-[#3A3B47] rounded-lg">
                        <form wire:submit.prevent="createTask({{$tasklist->id}})" class="space-y-3">
                            @if($userLists->count() === 0)
                            ...
                            @else
                            <input type="hidden" wire:model="newTaskListId.{{ $tasklist->id }}" 
                                   value="{{ $tasklist->id }}">
                            
                            @endif
                            <input type="text" wire:model="newTaskTitle.{{ $tasklist->id }}" 
                            ...
                            @error('newTaskTitle.{{ $tasklist->id }}') <span class="text-red-400 text-xs">{{ $message }}</span> @enderror
                            <textarea wire:model="newTaskDescription.{{ $tasklist->id }}" 
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <input type="date" wire:model="newTaskDueDate.{{ $tasklist->id }}" 
                                ...
                                <input type="time" wire:model="newTaskDueTime.{{ $tasklist->id }}" 
                                       class="w-full px-3 py-2 bg-[#2D2E3A] border border-[#3A3B47] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm">
                            </div>
                            ...
                            <div class="flex gap-2">
                                ...
                                <button type="button" 
                                        wire:click="closeAddForm({{ $tasklist->id }})"
                    ...
                    <div class="flex-1 space-y-3 overflow-y-auto tasks-list" data-list-id="{{ $listId ?? '' }}">
                        @if($list)
                        @forelse($this->getTasksByListId($tasklist->id) as $task)
                            ...
                            <div class="p-3 bg-[#353642] border border-[#3A3B47] rounded-lg hover:border-purple-500 transition-colors cursor-pointer task-item"
                                ...
                                <button wire:click.stop="toggleTaskComplete({{ $task->id }})"
                                            class="flex-shrink-0 mt-1 p-1.5 rounded-lg transition-all {{ $task->is_completed ? 'bg-green-500/30 text-green-400' : 'bg-[#3A3B47] text-gray-400 hover:bg-purple-500/30 hover:text-purple-400' }}"
